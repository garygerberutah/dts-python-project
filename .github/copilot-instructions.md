...
|-------|------|-----------|
| UI | Web Components (Shadow DOM, private `#fields`) | `ui/src/components/` |
| Rendering | WebGL via vanilla JS | `app-3d-viewer.js` |
| Math engine | Rust → WASM via wasm-bindgen | `ui/src/wasm/src/lib.rs` |
| Styles | SCSS (ITCSS) + Shadow DOM `<style>` | `ui/scss/` |
...
| Tool | Tech | Key Files |
|------|------|-----------|
| Build | Jinja2 templates → static HTML, wasm-pack, asset copy | `scripts/ui/build.py` |
| Lint | Python JS linter, cargo clippy, ruff | `scripts/ui/lint.py`, `scripts/ui/lint_js.py` |
| Format | rustfmt, ruff | `scripts/ui/format_code.py` |
| Test | pytest source-analysis | `tests/`, `tests/ui/` |
| Validate | WCAG 2.1 HTML parser | `scripts/ui/validate_wcag.py` |
| Pipeline | Orchestrates all stages | `scripts/build_all.py` |
...
cd ui/src/wasm && cargo test          # Rust unit tests
...
### Rust (WASM)
- The `ui/src/wasm/` directory is **exclusively Rust** — no hand-written JavaScript, TypeScript, or other languages may exist there; the only JS files are auto-generated by `wasm-pack` in `pkg/`
...
- The Python JS linter (`scripts/ui/lint_js.py`) enforces `eqeqeq`, `no-var`, `no-console.log` on UI JS
...
### Testing
- pytest + source-analysis fixtures in `tests/ui/conftest.py`
